...
const ITEMS = {
  ...
  boots: { name: "Ionian Boots", id: "3158" },
...
function analyzeBuild(enemies, tempo) {
  ...
  enemies.forEach((c) => {
    ...
    if (c.fed) {
      fedCount++;
      // tanks and bruisers can itemize defense easily
      if (c.tags.includes("tank") || c.tags.includes("bruiser")) fedCanItemize = true;
    }
  });
  ...
  // Double Tear check: only if 2+ fed threats who CAN'T itemize defense (squishies)
  const doubleTear = fedCount >= 2 && !fedCanItemize;

  // Core always: Trinity + boots + Manamune (boots free slot, 6 item slots)
  const core = [ITEMS.trinity, ITEMS.boots, ITEMS.manamune];
  let flex = []; // 4 flex slots (6 total items - trinity - manamune + boots is free)
  let reason = "";
  let tag = "";
  let tempoNote = "";
  let doubleTearNote = "";
  ...
  if (ahead) {
    if (isFullAD) {
      ...
      flex = [ITEMS.frozenHeart, ITEMS.endlessHunger, ITEMS.bt, ITEMS.serylda];
      reason = "You're ahead into full AD — Frozen Heart early is insane because defensive buys are strongest when snowballing. Armor + mana pumps Manamune damage while making you unkillable. Then Endless Hunger → BT → Serylda to close out.";
      tempoNote = "Lean into Frozen Heart early — you're ahead so the defensive buy locks in your lead.";
    } else if (isHeavyAP) {
      tag = "AHEAD vs HEAVY AP";
      flex = [ITEMS.maw, ITEMS.endlessHunger, ITEMS.bt, ITEMS.serylda];
      reason = "Ahead into heavy AP — grab Maw early to make their burst useless while you're snowballing. Defensive buys are strongest when ahead. Then full damage with Endless Hunger → BT → Serylda.";
      ...
      tag = "AHEAD vs TANKS";
      flex = [ITEMS.endlessHunger, ITEMS.bt, ITEMS.serylda, ITEMS.shojin];
      reason = "You're ahead vs tanks — go straight damage. Endless Hunger → BT gives highest DPS to melt through their HP. Serylda for armor shred, Shojin for ability haste to keep Qs flowing.";
      ...
      flex = [ITEMS.endlessHunger, ITEMS.bt, ITEMS.shojin, ITEMS.serylda];
      reason = "Ahead into bruisers — Endless Hunger → BT for max damage and sustain. You outstat them when ahead. Shojin + Serylda round out the build for haste and pen.";
      tempoNote = "Full damage first — you outstat them when ahead. Close the game out before they scale.";
    } else {
      tag = "AHEAD — FULL CARRY MODE";
      flex = [ITEMS.endlessHunger, ITEMS.bt, ITEMS.serylda, ITEMS.shojin];
      reason = "You're ahead with no extreme threats — go full damage. Endless Hunger → BT is the highest DPS carry path. You'll two-shot squishies and lifesteal through anything. Serylda + Shojin to round out.";
  ...
  // BEHIND
    ...
    if (isFullAD) {
      ...
      flex = [ITEMS.frozenHeart, ITEMS.serylda, ITEMS.endlessHunger, ITEMS.bt];
      reason = "You're behind into full AD — Frozen Heart is your lifeline. Armor keeps you alive, mana synergizes with Manamune. Serylda to stay relevant, then Endless Hunger + BT to scale back in.";
      tempoNote = "Survive first, damage later. Frozen Heart is non-negotiable here.";
    } else if (isHeavyAP) {
      tag = "BEHIND vs HEAVY AP";
      flex = [ITEMS.maw, ITEMS.serylda, ITEMS.endlessHunger, ITEMS.bt];
      reason = "Behind into heavy AP — Maw is mandatory or you'll get one-shot. Lifeline shield is the only thing keeping you in fights. Serylda so Qs still deal damage, then Endless Hunger + BT to scale.";
      tempoNote = "Maw is not optional — you will die without it. Play for poke with Q and don't E forward.";
    } else if (isTankHeavy || hasHPStackers) {
      tag = "BEHIND vs TANKS";
      flex = [ITEMS.serylda, ITEMS.endlessHunger, ITEMS.bt, ITEMS.shojin];
      reason = "Behind into tanks — you NEED Serylda's armor shred or your Qs will tickle. Without pen you're useless. Then Endless Hunger + BT to sustain, Shojin for haste to keep kiting.";
      tempoNote = "Serylda rush is mandatory — without armor pen you do zero damage to their frontline.";
    } else if (isBruiserHeavy) {
      tag = "BEHIND vs BRUISERS";
      flex = [ITEMS.shojin, ITEMS.serylda, ITEMS.endlessHunger, ITEMS.bt];
      reason = "Behind into bruisers — Shojin gives ability haste to kite and survive dives. Serylda for armor shred so you still deal damage. Endless Hunger + BT to scale back in.";
      tempoNote = "Haste and pen keep you relevant. Play for poke and don't commit with E.";
    } else {
      tag = "BEHIND — SAFE SCALING";
      flex = [ITEMS.serylda, ITEMS.endlessHunger, ITEMS.bt, ITEMS.shojin];
      reason = "You're behind — prioritize staying relevant. Serylda gives Qs armor shred to contribute from range. Then Endless Hunger + BT to scale back through sustain. Shojin last for haste.";
  ...
  // EVEN
    ...
    if (isFullAD) {
      tag = "FULL AD ENEMY";
      flex = [ITEMS.frozenHeart, ITEMS.endlessHunger, ITEMS.serylda, ITEMS.bt];
      reason = "Full AD comp detected. Frozen Heart gives massive armor + mana synergy with Manamune. Endless Hunger for damage, Serylda for shred, BT for sustain.";
    } else if (isHeavyAP) {
      tag = "HEAVY AP ENEMY";
      flex = [ITEMS.maw, ITEMS.endlessHunger, ITEMS.ldr, ITEMS.bt];
      reason = "Heavy AP threats. Maw gives lifeline shield to survive burst. Endless Hunger for sustain + damage, LDR if frontline builds armor, BT for lifesteal.";
    } else if (isTankHeavy || hasHPStackers) {
      tag = "TANK / HP STACKERS";
      flex = [ITEMS.serylda, ITEMS.endlessHunger, ITEMS.bt, ITEMS.shojin];
      reason = "Enemy is stacking HP and armor. Serylda 3rd gives armor shred on every Q. Endless Hunger + BT for sustained damage and lifesteal. Shojin for haste.";
    } else if (isBruiserHeavy) {
      tag = "BRUISER HEAVY";
      flex = [ITEMS.shojin, ITEMS.endlessHunger, ITEMS.bt, ITEMS.serylda];
      reason = "Multiple bruisers who will dive you. Shojin gives ability haste to kite them. Endless Hunger + BT for sustain through their all-ins. Serylda for pen.";
    } else {
      tag = "STANDARD / CARRY BUILD";
      flex = [ITEMS.endlessHunger, ITEMS.bt, ITEMS.serylda, ITEMS.shojin];
      reason = "No extreme threats — go full damage. Endless Hunger → BT is your highest DPS path. Serylda for armor shred, Shojin for haste.";
    }
  }
  ...
  return {
    ...
    flex,
    doubleTear,
    stats: { adCount, apCount, tankCount, bruiserCount, hpStackerCount, fedCount, total },
  };
}
...
function ItemCard({ item, label }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 68 }}>
      {label && <span style={{ fontSize: 9, color: "#C89B3C", marginBottom: 2, fontWeight: 600 }}>{label}</span>}
      <div style={{
...
function ChampIcon({ champ, size = 44, onClick, onRemove }) {
  const [imgErr, setImgErr] = useState(false);
  return (
    <div
      onClick={onClick}
...
export default function App() {
  // selectedEnemies stores { ...champData, fed: false }
  ...
  const addChamp = (c) => {
    if (selectedEnemies.length < 5) {
      setSelectedEnemies([...selectedEnemies, { ...c, fed: false }]);
      setSearch("");
    }
  };
  const removeChamp = (key) => setSelectedEnemies(selectedEnemies.filter((c) => c.key !== key));
  const toggleFed = (key) => setSelectedEnemies(selectedEnemies.map((c) =>
    c.key === key ? { ...c, fed: !c.fed } : c
  ));
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Selected enemies with fed toggle */}
        <div style={{ display: "flex", gap: 12, marginBottom: 16, minHeight: 72, flexWrap: "wrap" }}>
          {selectedEnemies.map((c) => (
            <div key={c.key} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
              <ChampIcon champ={c} size={50} onRemove={() => removeChamp(c.key)} />
              <span style={{ fontSize: 10, color: "#A09B8C" }}>{c.name}</span>
              <button
                onClick={() => toggleFed(c.key)}
                style={{
                  fontSize: 10, padding: "2px 8px", borderRadius: 4, cursor: "pointer",
                  border: c.fed ? "1px solid #E84057" : "1px solid #3C3C50",
                  background: c.fed ? "#E8405722" : "transparent",
                  color: c.fed ? "#E84057" : "#3C3C50",
                  fontWeight: c.fed ? 700 : 400,
                  transition: "all 0.15s",
                }}
              >{c.fed ? "FED" : "fed?"}</button>
            </div>
          ))}
          ...
          <div style={{ position: "relative" }} ref={dropdownRef}>
            ...
            {dropdownOpen && (
              <div style={{
                ...
                maxHeight: 280, overflowY: "auto", background: "#13132B",
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
            ...
            {build.doubleTear && (
              <div style={{
                ...
                color: "#A78BFA", fontSize: 13, fontWeight: 700, letterSpacing: 1,
              }}>DOUBLE TEAR</div>
            )}
          </div>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
            {[
              ...
              { label: "Fed", val: build.stats.fedCount, color: "#E84057" },
            ].map((s) => (
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 4, flexWrap: "wrap" }}>
              {build.startPath.map((item, i) => (
                <div key={item.name + i} style={{ display: "flex", alignItems: "center" }}>
                  <ItemCard item={item} />
          ...
          {/* Core (Trinity + Boots + Manamune) */}
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 11, color: "#C89B3C", marginBottom: 8, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1 }}>
              Core (Never Skip) — Boots are free slot
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 4, flexWrap: "wrap" }}>
              {build.core.map((item, i) => (
                <div key={item.name + i} style={{ display: "flex", alignItems: "center" }}>
                  <ItemCard item={item} label={item.name === "Ionian Boots" ? "FREE SLOT" : undefined} />
          ...
          {/* Double Tear note */}
          {build.doubleTearNote && (
            <div style={{
              marginTop: 10, background: "#A78BFA11", borderRadius: 8, padding: 12,
              border: "1px solid #A78BFA33",
            }}>
              <div style={{ fontSize: 11, fontWeight: 600, marginBottom: 4, color: "#A78BFA" }}>
                ⚡ DOUBLE TEAR ACTIVE
              </div>
              <p style={{ margin: 0, fontSize: 13, lineHeight: 1.5, color: "#CDBF9B" }}>
                {build.doubleTearNote}
          ...
          <div style={{
            ...
            <span style={{ fontWeight: 700, color: "#C89B3C" }}>Full 6-item path: </span>
            {[...build.core, ...build.flex].map((item, i, arr) => (
              <span key={i}>
                {item.name}{i < arr.length - 1 ? " → " : ""}
              </span>
            ))}
            <span style={{ color: "#A09B8C", fontSize: 10 }}> (boots = free slot)</span>
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        Core path is always Trinity Force → Boots → Manamune. Never sell Doran's Blade until final item slot.
        <br />Don't forget to Q Krugs/Gromp on your way back to lane. Mark enemies as "fed" to adjust the build.
      </div>
    </div>
  );
}
